...
def _count_param(block, prefix):
    """Count the params startign with a given prefix."""

    return sum(1 for p in block.param if p.startswith(prefix))

def draw_dag(dag, plain=True):
    ...
    text = fig.text(source=data, x='x', y='y', text='name', anchor='center', color='white', background_fill_color='black', background_fill_alpha=0.25, alpha=0.5, level=RenderLevel.annotation)
...
def dag_pane(dag, plain=True):
    return pn.pane.Bokeh(draw_dag(dag, plain))
